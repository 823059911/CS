...
//      board.move(c, r, t) 是已经计算好要移动到这个位置的了
    ...
    public static boolean maxTileExists(Board b){
        for (Tile t : b) { /** for (Tile t : b) 是遍历了 Board 对象 b中的 Tile 元素，将当前元素赋值给 t*/
    ...
    public static boolean atLeastOneMoveExists(Board b) {
            // 求出这个board一共多少行、列
            return emptySpaceExists(b) || mergeMoveExists(b);/**调用出现null的函数*/
        }
   public static boolean mergeMoveExists(Board b){
            ...
            for (int i = 0; i < numrow; i++) {
                for (int j = 0; j < numcol; j++) {
                    int KeyValue = b.tile(i, j).value(); // 检查瓦片的值
                    // 检查上方元素是否与其相等，i+1<numrow保证了不会越界
                    if (i + 1 < numrow && KeyValue == b.tile(i + 1, j).value()) {
                        return true;
                    }
                    // 检查右方元素是否与其相等，j+1<numcol保证了不会越界
                    if (j + 1 < numcol && KeyValue == b.tile(i, j + 1).value()) {
                        return true;
                    }
                }
            }
